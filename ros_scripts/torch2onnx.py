...
def build_dynamic_shapes(input_names: list[str]) -> dict[str, dict[int, Any]]:
    from torch.export import Dim

    batch_dim = Dim("batch", min=1)
    return {name: {0: batch_dim} for name in input_names}
...
def export_onnx(
    ...
    export_kwargs: dict[str, Any] = {
        ...
        "dynamo": True,
        "dynamic_shapes": build_dynamic_shapes(input_names),
        "external_data": external_data,
    }

    print(f"Creating ONNX model with dynamo exporter: {output_path}")
