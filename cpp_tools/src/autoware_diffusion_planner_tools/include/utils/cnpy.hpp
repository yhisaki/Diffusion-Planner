  ...
  std::string mode = "w", bool compress = false)
{
  ...
  global_header +=
    (uint32_t)global_header_offset;  // relative offset of local file header, since it begins where
  ...
  footer += (uint32_t)(global_header_offset + nbytes_on_disk +
