...
void parse_zip64_sizes(
  const std::vector<char> & extra_field, uint32_t & compr_bytes, uint32_t & uncompr_bytes)
{
  if (extra_field.size() >= 4 && (compr_bytes == 0xFFFFFFFF || uncompr_bytes == 0xFFFFFFFF)) {
    ...
    if (extra_id == 0x0001 && extra_size >= 16) {  // ZIP64 extended info
      ...
      if (uncompr_bytes == 0xFFFFFFFF) {
        uncompr_bytes =
          static_cast<uint32_t>(*reinterpret_cast<const uint64_t *>(&extra_field[offset]));
        offset += 8;
      }
